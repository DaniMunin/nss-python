...
class Fase(EscenaPygame):
    def __init__(self, director, jugador1, jugador2):
        ...
        # E inicialmente estaran quietos
        jugador1.numPostura = QUIETO
        jugador2.numPostura = QUIETO
    ...
    def desplazarDecorado(self, desplazamiento, jugadorAMover):
        ...
        # Actualizamos las animaciones para que tambien se desplacen
        for animacion in self.animacionesDetras:
            # Hacemos que las animaciones de detras se muevan a la mitad de velocidad del scroll
            animacion.mover(-desplazamiento/2, 0)
        for animacion in self.animacionesDelante:
            animacion.mover(-desplazamiento, 0)
    ...
    def dibujar(self):
        ...
        for animacion in self.animacionesDetras:
            animacion.dibujar(self.pantalla)
        # Después la imagen de fondo
        self.pantalla.blit(self.image, self.rect, self.rectSubimagen)
        # Luego los enemigos
        self.grupoEnemigos.draw(self.pantalla)
        # Los jugadores
        self.grupoJugadores.draw(self.pantalla)
        # Y por ultimo, dibujamos las animaciones por encima del fondo
        for animacion in self.animacionesDelante:
            animacion.dibujar(self.pantalla)
